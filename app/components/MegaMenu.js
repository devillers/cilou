...
const menuItems = [
  ...
  
  {
    title:  'Blog',
    submenu: [
      { title: 'reflexologie', items: ['article 1', 'article 1', 'article 3'] },
      { title: 'shiatsu assi', items: ['article 1', 'article 1', 'article 3']},
      { title: 'relaxologie', items: ['article 1', 'article 1', 'article 3'] },
      { title: 'bien être', items: ['article 1', 'article 1', 'article 3'] },
      { title: 'autres ', items: ['article 1', 'article 1', 'article 3'] },
    ],
  },
  {title: 'prendre RDV'},
        ...
        <div className="hidden md:flex justify-center flex-1">
          <nav className="flex gap-8 items-center">
            {menuItems.map((item, i) => (
              ...
              >
                <button className="text-sm focus:outline-none uppercase leading-4 text-[#009992] hover:text-[#006878]">{item.title}</button>
              </div>
            ))}
          </nav>
        </div>
              ...
              <div key={index}>
                ...
                <ul className="uppercase space-y-1">
                  {col.items.map((subItem, idx) => (
                    <li key={idx} className=" uppercase text-[10px] cursor-pointer leading-6">
                      <button className="w-full text-left hover:text-[#009996] uppercase">{subItem}</button>
                    </li>
                  ))}
                </ul>
              </div>
      ...
      {mobileOpen && (
        <div className="md:hidden bg-white shadow-sm fixed top-0 left-0 w-full h-full overflow-y-auto z-50 ">
          <div className="flex justify-end p-4">
               <div className=" gap-2">
                 <img src="/logo.png" alt="Logo" className="absolute left-0 ml-4 h-10 w-auto" />
                {/* <div className="text-xl uppercase">Pause  <span className='text-[#bd9254]'>Reflexo</span></div> */}
          </div>
          ...
          <nav className="flex flex-col space-y-4 px-4 pb-6 mt-10">
            {menuItems.map((item, i) => (
              <div key={i}>
                <div className="font-semibold text-md mb-3 uppercase hover:text-[#009996]">{item.title}</div>
                {item.submenu && (
                    ...
                    {item.submenu.map((col, index) => (
                      <div key={index}>
                        ...
                        <ul className="space-y-1">
                          {col.items.map((subItem, idx) => (
                            <li key={idx} className="text-[10px] uppercase hover:text-[#009996] cursor-pointer leading-7">
